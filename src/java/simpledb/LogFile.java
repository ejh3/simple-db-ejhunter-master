...
//    int pageSize;
    ...
    */
    ...
    public synchronized void logTruncate() throws IOException {
        ...
        logNew.seek(0);
        logNew.writeLong((cpLoc - minLogRecord) + LONG_SIZE); // updated checkpoint location
    ...
    /** Rollback the specified transaction, setting the state (on disk)
        of the pages it updated to their pre-updated state. To preserve
    ...
    */
    public void rollback(TransactionId tid) throws NoSuchElementException, IOException {
        Set<Long> arg = new HashSet<>();
        arg.add(tid.getId());
        rollback(arg);
    }

    /** Rollback the specified transactions, setting the state (on disk) of
     the pages they updated to their pre-updated state. To preserve
     transaction semantics, this should not be called on
     transactions that have already committed (though this may not
     be enforced by this method.)

     @param tids The transactions to rollback
     */
    public void rollback(Set<Long> tids) throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                int numUndone = 0;

                // Backwards traversal: at beginning of loop rollbackOffset is at start of next LR
                long originalOffset = raf.getFilePointer();
                raf.seek(raf.length() - LONG_SIZE);
                long rollbackOffset = raf.readLong(); // start of most recent log record
                while (numUndone != tids.size()) {
                    raf.seek(rollbackOffset); // seek to start of record
                    int type = raf.readInt();
                    long record_tid = raf.readLong();
                    if (type == UPDATE_RECORD && tids.contains(record_tid)) {
                        Page before = readPageData(raf);
                        int tableId = before.getId().getTableId();
                        DbFile table = Database.getCatalog().getDatabaseFile(tableId);
                        table.writePage(before); // undo early flush
                        // discard to guard the undo against being overwritten by evict().
                        Database.getBufferPool().discardPage(before.getId());

                    } else if (type == BEGIN_RECORD && tids.contains(record_tid)) {
                        numUndone++;
                    } // else move on to next record
                    raf.seek(rollbackOffset - LONG_SIZE);
                    rollbackOffset = raf.readLong();
                }
    ...
    */
    ...
    /** Recover the database system by ensuring that the updates of
        ...
        All changes made are written directly to disk.
    */
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // Beginning of analysis: find checkpoint, jump to it and read it
                raf.seek(0);
                long cpOffset = raf.readLong();
                long redoStart = raf.length();
                Set<Long> incompleteXacts = new HashSet<>();
                Set<Long> committedXacts = new HashSet<>();
                if (cpOffset == NO_CHECKPOINT_ID) {
                    redoStart = LONG_SIZE; // first log record
                } else {
                    int numXactions = raf.readInt();
                    while (numXactions-- > 0) {
                        try {
                            long tid = raf.readLong();
                            long xStartOffset = raf.readLong();
                            redoStart = min(redoStart, xStartOffset);
                            incompleteXacts.add(tid);
                        } catch (EOFException e) {
                            return;
                        }
                    }
                ...
                // Analysis cont. iterate through log, updating incompleteXacts and committedXacts.
                while (raf.getFilePointer() < raf.length()) {
                    int type = raf.readInt();
                    long record_tid = raf.readLong();
                    switch (type) {
                        case BEGIN_RECORD:
                            incompleteXacts.add(record_tid);
                            break;
                        case COMMIT_RECORD:
                            committedXacts.add(record_tid);
                            incompleteXacts.remove(record_tid);
                            break;
                        case ABORT_RECORD:
                            incompleteXacts.remove(record_tid);
                            break;
                ...
                // Redo phase: redo committed transactions that might not have been flushed
                raf.seek(redoStart);
                while (raf.getFilePointer() < raf.length()) {
                    int type = raf.readInt();
                    long record_tid = raf.readLong();
                    if (type == UPDATE_RECORD && committedXacts.contains(record_tid)) {
                        Page before = readPageData(raf);
                        Page after = readPageData(raf);
                ...
                //
                rollback(incompleteXacts);
            }
         }
    }

    /**
     * Advances the file pointer through the data (if there is any) part of
     * the log as well as the end pointer. If called under the correct circumstances
     * (type and tid already read) the file pointer will be at the beginning of the
     * next log record after it returns.
     *
     * @param type the type to skip over.
     */
    private void skipLogData(int type) throws IOException {
        switch (type) {
            case UPDATE_RECORD:
                readPageData(raf);
                readPageData(raf);
            ...
            case CHECKPOINT_RECORD:
                int numXactions = raf.readInt();
                raf.skipBytes(numXactions * 2 * LONG_SIZE);
                break;
        }
        raf.skipBytes(LONG_SIZE);
    }
    ...
    public void print() throws IOException {
        long originalOffset = raf.getFilePointer();
        raf.seek(0);
        System.out.printf("Checkpoint is at %06d\n", raf.readLong());

        System.out.println("Oldest to most recent logs:");
        while (raf.getFilePointer() < raf.length()) {
            System.out.printf("%06d\t", raf.getFilePointer());
            int type = raf.readInt();
            long record_tid = raf.readLong();
            switch (type) {
                case ABORT_RECORD:
                    System.out.printf("ABORT:  %08X", record_tid);
                    break;
                case COMMIT_RECORD:
                    System.out.printf("COMMIT: %08X", record_tid);
                    break;
                case UPDATE_RECORD:
                    System.out.printf("UPDATE: %08X", record_tid);
                    break;
                case BEGIN_RECORD:
                    System.out.printf("BEGIN:  %08X", record_tid);
                    break;
                case CHECKPOINT_RECORD:
                    System.out.printf("CHECKPOINT");
                    long temp = raf.getFilePointer();
                    int numXactions = raf.readInt();
                    System.out.printf(" %d running: ", numXactions);
                    while (numXactions-- > 0) {
                        long xid = raf.readLong();
                        long xoffset = raf.readLong();
                        System.out.printf("T%d, ", xid);
                    }
                    raf.seek(temp);
                    break;
