...
public class LogTest extends SimpleDbTestBase {
    ...
    // Many thanks and all credit to Yu Xin for posting this additional test on ed here:
    // https://edstem.org/us/courses/32305/discussion/2738844
    @Test public void TestManyCheckpointManyCrash()
            throws IOException, DbException, TransactionAbortedException {
        setup();
        doInsert(hf1, 1, 2);
        ...
        // T1 inserts but does not commit
        // T2 inserts and commits
        // T3 & T4 insert and rollback
        // checkpoint
        // T5 inserts but does not commit
        // crash
        // checkpoint
        // T6 insert and commit
        // T7 insert and rollback
        // checkpoint
        // crash again
        // T8 insert and rollback
        // only T2 and T6 data should be there
        ...
        // T4 abort
        dontInsert(hf2, 200, 300);

        insertRow(hf2, t3, 100, 0);
        insertRow(hf2, t3, 200, 0);

        look(hf2, t3, 100, true);
        look(hf2, t3, 200, true);
        look(hf2, t3, 300, false);
        ...
        t5.start();
        insertRow(hf2, t5, 28, 0);
        Database.getBufferPool().flushAllPages(); // XXX defeat NO-STEAL-based abort
        insertRow(hf2, t5, 29, 0);
        ...
        t6.start();
        Database.getBufferPool().flushAllPages(); // flush without insert (why not?)
        insertRow(hf1, t6, 31, 0);
        t6.commit();

        Transaction t7 = new Transaction();
        t7.start();
        insertRow(hf2, t7, 40, 0);
        Database.getBufferPool().flushAllPages(); // XXX defeat NO-STEAL-based abort
        abort(t7);
        ...
        look(hf2, t, 40, false);
        look(hf2, t, 42, false);
        look(hf2, t, 100, false);
        look(hf2, t, 200, false);
        look(hf2, t, 300, false);
        t.commit();
    }
